...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class CopyTransparentPass : ScriptableRenderPass
    {
        ...
        public CopyTransparentPass(RenderPassEvent evt, Material samplingMaterial, Material copyColorMaterial = null)
        {
        ...
        internal TextureHandle Render(RenderGraph renderGraph, out TextureHandle destination, in TextureHandle source, Downsampling downsampling, ref RenderingData renderingData)
        {
            ...
            using (var builder = renderGraph.AddRenderPass<PassData>("Copy Transparent Color", out var passData, base.profilingSampler))
            {
                RenderTextureDescriptor descriptor = renderingData.cameraData.cameraTargetDescriptor;
                ConfigureDescriptor(downsampling, ref descriptor, out var filterMode);

                destination = UniversalRenderer.CreateRenderGraphTexture(renderGraph, descriptor, "_CameraTransparentTexture", true, filterMode);
            ...
            using (var builder = renderGraph.AddRenderPass<PassData>("Set Global Copy Transparent Color", out var passData, base.profilingSampler))
            {
                RenderTextureDescriptor descriptor = renderingData.cameraData.cameraTargetDescriptor;
                ConfigureDescriptor(downsampling, ref descriptor, out var filterMode);
                ...
                builder.SetRenderFunc((PassData data, RenderGraphContext context) =>
                {
                    data.cmd.SetGlobalTexture("_CameraTransparentTexture", data.destination);
                });
            }
