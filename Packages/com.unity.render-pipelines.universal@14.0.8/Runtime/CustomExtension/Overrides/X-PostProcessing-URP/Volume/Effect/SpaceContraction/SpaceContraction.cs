...
{
    [VolumeComponentMenu(VolumeDefine.Extra + "空间收缩 SpaceContraction")]
    public class SpaceContraction : VolumeSetting
    {
        ...
        {
            return "Hidden/PostProcessing/SpaceContraction";
        }
    }
    ...
    public class SpaceContractionRenderer : VolumeRenderer<SpaceContraction>
    {
        ...
        public const string PROFILER_TAG = "Space Contraction";
        ...
        public override void Render(CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier target, ref RenderingData renderingData)
        {
            ...
            m_BlitMaterial.SetFloat(ShaderContants.raderMaxDistanceID, settings.maxDistance.value);
            m_BlitMaterial.SetFloat(ShaderContants.raderWaveAreaColorID, settings.raderAreaColor.value);
            m_BlitMaterial.SetFloat(ShaderContants.areaBrightnessID, 1);
            m_BlitMaterial.SetFloat(ShaderContants.raderWaveIntensityID, settings.raderWaveIntensity.value);
            m_BlitMaterial.SetFloat(ShaderContants.raderWaveWidthID, settings.raderWaveWidth.value);
            m_BlitMaterial.SetFloat(ShaderContants.gridStartTimeID, settings.gridStartTime.value);
            m_BlitMaterial.SetFloat(ShaderContants.gridRadiusID, settings.gridRadius.value);
            m_BlitMaterial.SetFloat(ShaderContants.gridSpeedID, settings.gridSpeed.value);
            m_BlitMaterial.SetColor(ShaderContants.curveColorID, settings.curveColor.value);
            m_BlitMaterial.SetColor(ShaderContants.gridColorID, settings.gridColor.value);
            m_BlitMaterial.SetFloat(ShaderContants.gridLengthID, settings.gridLength.value);
            m_BlitMaterial.SetFloat(ShaderContants.raderwaveTimeLineID, settings.raderWaveTimeLine.value);
            m_BlitMaterial.SetFloat(ShaderContants.raderwaveTimeCountID, settings.raderWaveTime.value);
            m_BlitMaterial.SetVector(ShaderContants.positionBufferID, settings.positionBuffer0.value);
            m_BlitMaterial.SetVector(ShaderContants.positionBuffer1ID, settings.positionBuffer1.value);
