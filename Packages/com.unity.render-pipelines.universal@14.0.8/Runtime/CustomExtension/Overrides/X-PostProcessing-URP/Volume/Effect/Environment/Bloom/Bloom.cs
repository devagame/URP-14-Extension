...
namespace XPostProcessing
{
    ...
    public class Bloom : VolumeSetting
    {
        ...
        {
            return "Hidden/PostProcessing/Environment/Bloom";
        }
    }

    public class BloomRenderer : VolumeRenderer<Bloom>
    {
        public const string PROFILER_TAG = "Bloom";
       ...
       // public override bool CheckActive(ref RenderingData renderingData) => IsEnable;
            ...
            ref RenderingData renderingData)
        {
            ...
            m_BlitMaterial.SetVector(ShaderIDs.ParamsID, param);
            m_BlitMaterial.SetFloat(ShaderIDs.SampleScaleID, settings.SampleScale.value);
            cmd.Blit(source, ShaderIDs.PrefilterID, m_BlitMaterial, 0);
            ...
            for (int i = 0; i < mipCount; i++)
            {
                ...
                if (settings.BloomQuailty.value == BloomQuailtyType.Low)
                    cmd.Blit(lastDown, mipDown, m_BlitMaterial, 1);
                else
                {
                    cmd.Blit(lastDown, mipDown, m_BlitMaterial, 4);
                    cmd.Blit(mipDown, lastDown, m_BlitMaterial, 5);
                }
            ...
            for (int i = mipCount - 1; i >= 0; i--)
            {
                ...
                cmd.Blit(highMip, dst, m_BlitMaterial, 1);
                lastDown = dst;
            }

            //Final
            cmd.GetTemporaryRT(ShaderIDs.BloomTexID, RTWidth << 1, RTHeight << 1, 0, FilterMode.Bilinear);
            m_BlitMaterial.SetFloat(ShaderIDs.IntensityID, settings.Intensity.value);
            cmd.Blit(lastDown, ShaderIDs.BloomTexID, m_BlitMaterial, 2);

            //Combine
            cmd.SetGlobalTexture(ShaderIDs.SourceTexID, source);
            cmd.Blit(ShaderIDs.BloomTexID, target, m_BlitMaterial, 3);
