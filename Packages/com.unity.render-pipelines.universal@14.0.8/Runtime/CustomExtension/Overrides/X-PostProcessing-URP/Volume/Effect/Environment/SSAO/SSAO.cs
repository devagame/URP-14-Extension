...
namespace XPostProcessing
{
    ...
    public class SSAO : VolumeSetting
    {
        ...
        {
            return "Hidden/PostProcessing/Environment/SSAO";
        }
    }
    ...
    {
        public const string PROFILER_TAG = "SSAO";
        ...
        public override void Render(CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier target, ref RenderingData renderingData)
        {
            switch (settings.NormalQuality.value)
            {
                case SSAONormalQuality.Low:
                    CoreUtils.SetKeyword(m_BlitMaterial, ShaderIDs.k_NormalReconstructionLowKeyword, true);
                    CoreUtils.SetKeyword(m_BlitMaterial, ShaderIDs.k_NormalReconstructionMediumKeyword, false);
                    CoreUtils.SetKeyword(m_BlitMaterial, ShaderIDs.k_NormalReconstructionHighKeyword, false);
                    break;
                case SSAONormalQuality.Medium:
                    CoreUtils.SetKeyword(m_BlitMaterial, ShaderIDs.k_NormalReconstructionLowKeyword, false);
                    CoreUtils.SetKeyword(m_BlitMaterial, ShaderIDs.k_NormalReconstructionMediumKeyword, true);
                    CoreUtils.SetKeyword(m_BlitMaterial, ShaderIDs.k_NormalReconstructionHighKeyword, false);
                    break;
                case SSAONormalQuality.High:
                    CoreUtils.SetKeyword(m_BlitMaterial, ShaderIDs.k_NormalReconstructionLowKeyword, false);
                    CoreUtils.SetKeyword(m_BlitMaterial, ShaderIDs.k_NormalReconstructionMediumKeyword, false);
                    CoreUtils.SetKeyword(m_BlitMaterial, ShaderIDs.k_NormalReconstructionHighKeyword, true);
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }

            m_BlitMaterial.SetVector(ShaderIDs.Params, new Vector4(settings.sampleRadius.value, settings.sampleCount.value, settings.AOIntensity.value, settings.blurRadius.value));
            ...
            cmd.Blit(source, ShaderIDs.TempBlurBuffer1, m_BlitMaterial, 0);//计算AO

            cmd.GetTemporaryRT(ShaderIDs.TempBlurBuffer2, RTWidth, RTHeight, 0);
            cmd.Blit(ShaderIDs.TempBlurBuffer1, ShaderIDs.TempBlurBuffer2, m_BlitMaterial, 1);//H模糊
            cmd.Blit(ShaderIDs.TempBlurBuffer2, ShaderIDs.TempBlurBuffer1, m_BlitMaterial, 2);//V模糊

            if (settings.debug.value)
            {
                cmd.Blit(ShaderIDs.TempBlurBuffer1, target, m_BlitMaterial, 4);//塞回去
            }
            else
            {
                cmd.SetGlobalTexture(ShaderIDs.ScreenTex, source);
                cmd.Blit(ShaderIDs.TempBlurBuffer1, target, m_BlitMaterial, 3);//塞回去
            }
