Shader "Hidden/PostProcessing/Skill/BlackWhite5"
{
    Properties
    {
    ...
    #define jitterX _Params4.w
    #define jitterY _Params5.x
    ...
    half4 Frag(VaryingsDefault input): SV_Target
    {
        ...
        noiseUV = float2(input.uv.xy + float2(jitter,0)) * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
        //noiseUV = float2(input.uv.x,input.uv.y) * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
